...
- **Asset generation fan-out:** `analyze_intelligence` enqueues one `generate_asset` per type in `ASSET_TYPES` (always-on: youtube_*, linkedin_post, x_post/thread, article_brief, newsletter_summary, youtube_pinned_comment, short_clip_plan, long_clip_plan). **Extended social networks are gated** — `NETWORK_ASSET_TYPES` (facebook_post, threads_post, pinterest_pin, reddit_post, bluesky_post, telegram_post, discord_message, google_business_post) generate ONLY when `brands.zernio_accounts[network]` is set, so we never draft posts a brand can't publish. To add an asset type see [[adding-an-asset-type]]; dispatch auto-routes anything in `NETWORK_BY_TYPE` to Zernio via `isZernioDispatchable()`. `long_clip_plan` ends in `_plan` so the auto-render fan-out + `clip_render` produce a horizontal `rendered_long_clip` with no extra wiring.
...
- **Performance & feedback features:** `/performance` is a read-only cross-surface view of how dispatched/published assets performed (`signals` + A/B results) — no mutations, just reads. `collect_signal` has a `dojoclaw` branch (foundation only — degrades to a no-op until DojoClaw exposes an analytics endpoint; GSC OAuth is still deferred). `mineComments` is a post-publish, on-demand action (Studio "Mine comments" panel) that pulls a video's top YouTube comments → `content_ideas` + `faq` assets; these are content-loop outputs and are deliberately NOT in `ASSET_TYPES` (they don't generate in the bulk pipeline or dispatch). Brand-voice bootstrap (`/brands/[id]/voice`) seeds `voice_examples` from pasted samples or existing published assets; shared consts/types for it live in `src/lib/voice-types.ts` because a `'use server'` file may only export async functions. `translateClipSubtitles` writes per-clip `subtitle_translations` (per-language SRT + ASS sidecar files); TTS dubbing and a burned-in per-language re-render are deferred.
